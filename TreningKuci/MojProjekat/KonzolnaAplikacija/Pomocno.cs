...
namespace MojProjekat.KonzolnaAplikacija
{
    internal class Pomocno
    {
        ...
        internal static DateOnly UcitajDatum(string poruka)
        {
            string unos;
            while (true)
            {
                try
                {
                    Console.WriteLine("Format unosa je dd-MM-yyyy, za današnji datum {0}");
                    Console.Write(poruka + ": ");
                    unos = Console.ReadLine();

                    if (DateOnly.TryParseExact(unos, "dd-MM-yyyy", null, System.Globalization.DateTimeStyles.None, out DateOnly d))
                    {
                        return d;
                    }
                    throw new Exception();
        ...
        internal static float UcitajDecimalniBroj(string poruka, float min, float max)
        {
            float b;
            while (true)
            {
        ...
        internal static float UcitajDecimalniBroj(string poruka, float min, float max,bool obavezno)
        {
            float b;
            string s;
            while (true)
